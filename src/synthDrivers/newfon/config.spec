...
# Замены отдельных символов в русском тексте. Делаются после словаря RuLex,
# непосредственно перед передачей текста ядру. Ядро само читает большую часть
# латиницы, как оригинальный Newfon (q как «к», w как «в»), здесь перечислены
# буквы, которые оно читает неверно. Можно добавлять свои строки вида символ = замена
[Characters]
j = string(default=дж)
x = string(default=кс)

# Названия букв для русского текста: когда текст состоит из одной буквы,
# когда латинская буква стоит отдельно и когда аббревиатура из одних
# согласных читается по буквам. Можно добавлять свои строки вида буква = название
[SingleCharacters]
б = string(default=бэ)
в = string(default=вэ)
с = string(default=эс)
