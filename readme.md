...
### Версия 2026.09.11
* Оптимизация модулей. Теперь все Русские и Английские буквы берутся из ini- файла. Ранее они были и в модулях ru и en.
* Теперь символы, которые движок читать не может и их нет ни в одной секции newfon.ini, не попадают в речевой поток.

### Версия 2026.09.10
* Исправлена ошибка: при произнесении таких слов как "qwen" произносилось "кувен"
* Теперь используется одна база rulex.
* Убрана настройка "Псевдоанглийское произношение", т.к. теперь е на э можно поменять в ini- файле.
* Теперь все настройки должны нормально работать и применяться.
* Прочие улучшения

### Версия 2026.08.21
* Ядро теперь собирается из исходников. Всем известно, что [ruTTS](https://github.com/poretsky/ru\_tts) и newfon- по сути одно и тоже, а посему воссоздать недостающее (4 голоса) было несложно.
...
* Теперь некоторые параметры живут в ini- файле, который, если его нет, будет создан при первом использовании синтезатора. Файл лежит по пути "C:\\\\Users\\\\<username>\\\\AppData\\\\Roaming\nvda". В этом файле имеются такие параметры: samples\\\_per\\\_sec- частота выходного сигнала, interpolation\\\_multiplier- интерполяция, interpolation\\\_algorithm- алгоритм интерполяции. Также как и в ru\\\_TTS имеются секции Characters, SingleCharacters.
